...
API lives at /api/*. Static site served from repo root at /*.
"""
...
def depth_to_normals(d, strength=1.0):
    ...
    dx = sobel(f, axis=1) * strength
    dy = sobel(f, axis=0) * strength
...
def est_roughness(a):
    ...
    r = np.sqrt(v)
    r = np.clip(r / (r.max() + 1e-8), 0.05, 0.98)
...
async def depth(file: UploadFile = File(...)):
    try:
        ...
        if not isinstance(d, Image.Image):
            d = Image.fromarray(np.array(d))
...
async def normals(file: UploadFile = File(...), strength: float = 2.0):
    try:
        ...
        if isinstance(d, Image.Image):
            dn = np.array(d.convert("L").resize(img.size, Image.BILINEAR))
        else:
            dn = np.array(d)
            if dn.ndim == 3:
                dn = dn[:, :, 0]
...
# API routes first (matched before static mount)
app.include_router(api)

# Static site from repo root (parent of material-lab-server/)
SITE_ROOT = Path(__file__).resolve().parent.parent
if (SITE_ROOT / "stella-nova").is_dir():
    app.mount("/", StaticFiles(directory=str(SITE_ROOT), html=True), name="site")
    log.info(f"Static site: {SITE_ROOT}")
else:
    log.warning(f"stella-nova/ not found at {SITE_ROOT}")

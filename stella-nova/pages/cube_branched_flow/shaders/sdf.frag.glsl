...
//   PER-PIXEL PATH
//   --------------------------------------------------------------------------
//     build ray ─▶ marchHi() 70 steps ─▶ hit?
//        u_mode==1  ─▶ output depth t/500 in red   (distance pass for occlusion)
...
#define MD 500.0
// Globals shared across the march: g_gd = nearest torus distance (for glow),
// gR = the animated scene rotation, g_i = steps used by the last march (for AO).
float g_gd;mat3 gR;int g_i;
const vec4 hK=vec4(1,.6667,.3333,3);
// HSV to RGB, and the ACES filmic tonemap used before gamma.
...
// Smooth-min / smooth-max / smooth-abs primitives for soft CSG on the SDF.
...
// Primary march, up to 70 steps; keeps the closest approach as a fallback hit
// and records the step count in g_i (used as a cheap ambient-occlusion cue).
...
// Cheaper 30-step march for the reflection ray.
...
// Cheap gradient environment for the background and reflection misses.
...
void main(){vec2 uv=(gl_FragCoord.xy-.5*u_res)/u_res.y;vec3 ro=u_ro;
  ...
  // Distance pass: pack normalized depth into red for the occlusion texture.
  ...
  // Volumetric torus glow: brightness falls as inverse-sqrt of nearest distance.
  vec3 ggc=gC*inversesqrt(max(gd,.00025));vec3 col=env(rd)*u_mEnv;
  if(t<MD){vec3 p=ro+rd*t,n=calcN(p);
    // Brushed-metal micro-normal perturbation for anisotropic highlights.
    float brush=sin(dot(p,normalize(vec3(1.7,2.3,.9)))*40.)*u_mBrush;
    vec3 bn=normalize(n+cross(n,normalize(vec3(.3,1,.7)))*brush*.1);
    // Reflection direction and fresnel; step-count gives a cheap AO factor.
    ...
    // Reflection ray: a low-step march plus its own torus glow, then the
    // filaments reflected via a roughness-scaled offset into u_filTex.
  ...
  // Add the primary glow, filmic tonemap, then gamma to sRGB.

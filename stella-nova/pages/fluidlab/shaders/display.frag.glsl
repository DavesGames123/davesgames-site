#version 300 es
// ============================================================================
//  display.frag.glsl — final render pass (fields to screen)
// ----------------------------------------------------------------------------
//  Turn the simulation state into pixels. Barrier cells draw as a dark solid
//  with a lit edge; fluid cells are coloured by the chosen colormap. Modes 0-2
//  map flow speed through the Predator / Viridis / Oceanic palettes; mode 3
//  ("Dye") shows the carried dye colour directly. uGain scales brightness and
//  uBloom adds a simple self-add glow.
//
//  This is a JS template literal: main.js injects a manual bilinear sampler
//  (bilerp2) and swaps the sampling expressions where hardware float filtering
//  is unavailable. The cmP / cmV / cmO functions mirror the CMAPS table in JS.
//
//  SECTION MAP   (grep -n "<anchor>" display.frag.glsl)
//      colormaps ... "vec3 cmP"     speed palettes (GPU copies of CMAPS)
//      barrier ..... "if(barrier"   dark fill plus edge highlight
//      fluid ....... "else{"        speed-to-colour or dye, then bloom
// ============================================================================
...
// Predator / Viridis / Oceanic speed palettes: 8 stops each, interpolated by t.
...
void main(){vec2 uv=vUV;float barrier=texture(uBarrier,uv).r;${TEX_CAN_LINEAR?'vec2 vel=texture(uVelocity,uv).xy;vec3 dye=texture(uDye,uv).rgb;':'vec2 vel=bilerp2(uVelocity,uv).xy;vec3 dye=bilerp2(uDye,uv).rgb;'}vec3 col;
  // Barrier: dark fill, with a cyan edge where the mask changes (its gradient).
  ...
  // Fluid: map log-compressed speed through the colormap, or show dye directly in
  // mode 3, then add the bloom self-glow.
